...
struct args
{
    ...
    pthread_mutex_t *mutexs; // Array de mutex
};
...
void *swap(void *ptr)
{
    ...
    while (args->iterations--)
    {
        ...
        if (i != j)
        {
            pthread_mutex_lock(&args->mutexs[primero]);
            pthread_mutex_lock(&args->mutexs[segundo]);
        }
        else
        {
            pthread_mutex_lock(&args->mutexs[primero]);
        ...
        if (i != j)
        {
            pthread_mutex_unlock(&args->mutexs[segundo]);
            pthread_mutex_unlock(&args->mutexs[primero]);
        }
        else
        {
            pthread_mutex_unlock(&args->mutexs[primero]);
...
void start_threads(struct options opt)
{
    ...
    mutexs = malloc(sizeof(pthread_mutex_t) * opt.buffer_size);
