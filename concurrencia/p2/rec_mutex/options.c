...
int handle_options(int argc, char **argv, struct options *opt)
{
    while (1)
    {
        ...
        {
        case 't':
            if (!get_int(optarg, &opt->num_threads) || opt->num_threads <= 0)
            {
        ...
        case 'b':
            if (!get_int(optarg, &opt->buffer_size) || opt->buffer_size <= 0)
            {
                printf("'%s': is not a valid integer\n",
                       optarg);
                usage(-3);
        ...
        case 'i':
            if (!get_int(optarg, &opt->iterations) || opt->iterations <= 0)
            {
                printf("'%s': is not a valid integer\n",
                       optarg);
                usage(-3);
        ...
        case 'd':
            if (!get_int(optarg, &opt->delay) || opt->delay <= 0)
            {
                printf("'%s': is not a valid integer\n",
                       optarg);
                usage(-3);
        ...
        case 'p':
            if (!get_int(optarg, &opt->print_wait) || opt->print_wait <= 0)
            {
                printf("'%s': is not a valid integer\n",
                       optarg);
                usage(-3);
